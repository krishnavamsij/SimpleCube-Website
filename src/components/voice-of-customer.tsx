...
const getTagIcon = (tag: string) => {
    if (tag.includes("Lending") || tag.includes("Fintech") || tag.includes("Financial") || tag.includes("Credit")) return <Landmark className="w-4 h-4 text-[#3886CE]" />;
    if (tag.includes("Insurance")) return <Shield className="w-4 h-4 text-[#3886CE]" />;
    if (tag.includes("Wealth")) return <TrendingUp className="w-4 h-4 text-[#3886CE]" />;
    if (tag.includes("Education")) return <GraduationCap className="w-4 h-4 text-[#3886CE]" />;
    if (tag.includes("Logistics") || tag.includes("Transportation")) return <Truck className="w-4 h-4 text-[#3886CE]" />;
    ...
    return (
        <section className="bg-white pt-[10px] pb-[30px] sm:pt-[20px] sm:pb-[40px] lg:pt-[20px] lg:pb-[50px] relative overflow-hidden min-h-0 lg:min-h-[600px] xl:min-h-[750px] flex flex-col justify-center">
                ...
                <div className="absolute top-[68%] lg:top-[55%] left-[60%] lg:left-[70%] -translate-y-1/2 w-full h-full flex items-center justify-center">
                    {[1, 2, 3, 4, 5].map((i) => (
                        <motion.div
                            ...
                            className="absolute rounded-full border-[2px] border-[#3886CE]/30"
                            style={{
            ...
            <div className="mx-auto max-w-[96rem] px-6 md:px-10 lg:px-16 w-full relative z-10 pb-16 lg:pb-8">
                ...
                >
                    <div className="inline-flex items-center gap-2 text-[11px] font-bold tracking-[2px] uppercase text-[#3886CE] bg-[#3886CE]/[0.08] border border-[#3886CE]/25 rounded-full px-5 py-1.5 mb-6">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#3886CE] shadow-[0_0_8px_#3886CE] animate-pulse" />
                        Customer Stories
                    </div>
                    <h2 className="text-[32px] sm:text-[44px] lg:text-[52px] font-extrabold tracking-tight text-[#0A2F52] leading-[1.1] max-w-2xl relative z-10">
                        {headline.split(" ").map((word, i) => {
                            ...
                            return (
                                <span key={i} className={isHighlighted ? "text-[#3886CE]" : ""}>
                                    {word}{" "}
                                </span>
                            );
                        })}
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            <div className="flex flex-col items-start space-y-8 pt-4 lg:col-span-8">
                                <div className="inline-flex items-center gap-2.5 rounded-full bg-[#3886CE]/5 border border-[#3886CE]/20 px-6 py-3 text-xs font-semibold text-[#3886CE] tracking-wide">
                                    {getTagIcon(active.industry)}
                                    {active.industry}
                                </div>

                                <div className="relative min-h-[100px] flex items-center">
                                    <p className="text-lg md:text-xl lg:text-2xl xl:text-[26px] font-medium leading-relaxed text-[#0A2F52] relative z-10 text-left max-w-[92%]">
                                ...
                                {active.result && (
                                    <div className="pt-2 w-full max-w-[92%]">
                                        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 p-6 rounded-2xl bg-[#3886CE]/5 border border-[#3886CE]/20 w-full min-h-[96px]">
                                            <p
                                                className="text-sm sm:text-base text-[#0A2F52] font-medium leading-relaxed flex-1"
                                                dangerouslySetInnerHTML={{ __html: active.result as string }}
                                            />
                                            {active.videoUrl ? (
                                                <button
                                                    onClick={() => setIsVideoModalOpen(true)}
                                                    className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full bg-gradient-to-r from-[#3886CE] to-[#135498] text-white font-bold text-xs shadow-[0_0_15px_rgba(56,134,206,0.3)] border border-[#3886CE]/30 transition-all duration-300 hover:opacity-90 hover:shadow-[0_0_20px_rgba(56,134,206,0.6)] group uppercase tracking-wide shrink-0 whitespace-nowrap"
                                                >
                                ...
                                <div className="flex flex-col items-center w-full">
                                    ...
                                    >
                                        ...
                                        {[0, 1].map((i) => (
                                            <motion.div
                                                ...
                                                className="absolute inset-0 rounded-full bg-[#3886CE]/20 blur-2xl z-0"
                                            />
                                        ))}
                                        ...
                                        <div className="relative w-full h-full overflow-hidden rounded-full ring-4 ring-white shadow-2xl bg-transparent z-10">
                                            ...
                                            {active.videoUrl && (
                                                <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 z-20 ${isHovered ? "opacity-0 pointer-events-none" : "opacity-100 bg-black/20 group-hover:bg-black/30"
                                                    ...
                                                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center bg-white border border-white/80 shadow-[0_0_30px_rgba(0,0,0,0.3)] text-[#3886CE] transition-all duration-300 scale-95 group-hover:scale-105 group-hover:shadow-[0_0_40px_rgba(30,144,255,0.6)]">
                                                        <Play className="w-7 h-7 sm:w-8 sm:h-8 fill-[#3886CE] stroke-none ml-1 transition-transform duration-300 group-hover:scale-110" />
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    <div className="mt-8 text-center flex flex-col items-center justify-start min-h-[110px]">
                                        <div className="space-y-1">
                                            <h4 className="text-2xl sm:text-3xl font-bold text-[#0A2F52] leading-tight tracking-tight">{active.author}</h4>
                ...
                <div className="relative lg:absolute mt-8 lg:mt-0 lg:bottom-4 left-0 lg:left-1/2 w-full lg:w-auto flex justify-center lg:-translate-x-1/2 gap-3 z-30">
                    {testimonials.map((_, i) => (
                        <button
                            ...
                            className={`h-1.5 rounded-full transition-all duration-500 ${i === current ? "w-10 bg-[#3886CE]" : "w-1.5 bg-[#3886CE]/20"
                                }`}
